...
Range within the close range zone, including to and from melee, is 25 ft (e.g. poison spray spell, normal range for a javelin, or max range for a dagger). You can deliver touch spells between two friendly characters in the close range zone, but to touch an enemy (or a friendly in melee), you also need to be in melee (e.g. shocking grasp or thunderwave spells, normal range for a dagger, or melee attacks).
...
To get get from out of combat to engage an enemy at long range requires: move to long range (2 moves), move to close range (1 move), move focus to the enemy (1 move, moves them to close range), engage the enemy (1 move), for a total of 5 standard moves.

Fleeing from melee requires: withdraw from melee (1 move, with opportunity attack), move to long range (1 move), move to out of combat (2 moves), for a total of 4 standard moves (plus you may want to take an action to disengage).
...
e.g. If there are 3 friendlies in melee, and 4 enemies, then withdrawing would draw 2 opportunity attacks. If there were 7 enemies it would be 3 opportunity attacks.
...
Area spells generally only affect one zone (long range, close range, or melee). Cones and cubes 40 ft or greater, or radius of 20 ft or greater, can affect both close range and melee at the same time.

Long lines spread their effects across multiple zones based on their length and the zone ranges. e.g. lightning bolt could affect 1 creatures in melee, 2 in close range, and 1 at long, but can't affect 4 creatures in melee.
...
For each additional enemy that is affected in melee beyond the first, one friendly is also within the area of effect.

E.g. A fireball could get 2 enemies in close range and 3 enemies in melee, but would also catch 2 friendly creatures in the melee.
